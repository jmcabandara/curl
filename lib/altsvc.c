...
static const char *alpnid2alpn(enum alpnid id)
{
  switch(id) {
  case ALPN_h1:
    return "h1";
...
static CURLcode altsvc_out(struct altsvc *as, FILE *fp)
{
  ...
  fprintf(fp,
          ...
          "%02d:%02d:%02d\" "
          "%u %d\n",
          alpnid2alpn(as->srcalpnid), as->srchost, as->srcport,
          alpnid2alpn(as->dstalpnid), as->dsthost, as->dstport,
  ...
  do {
    if(*p == '=') {
      ...
      if(*p == '\"') {
        ...
        if(*p++ != '\"')
          return CURLE_BAD_FUNCTION_ARGUMENT;
        as = altsvc_createid(srchost, host,
                             srcalpnid, dstalpnid,
                             srcport, dstport);
        if(as) {
